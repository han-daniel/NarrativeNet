...
document.getElementById('textForm').addEventListener('submit', function(e) {
    ...
    if (!text.trim()) {
        document.getElementById('results').innerHTML = 'Error: No text provided';
        return;
    }

    try {
        const data = NarrativeNet.analyzeText(text);
        if (data.characters.length === 0) {
            document.getElementById('results').innerHTML = 'No characters found in the text.';
            return;
        }
        lastResult = data;
        createMetricsTable(data.metrics);
        renderNetwork(data.d3Network);
    } catch (error) {
...
document.getElementById('exportNetwork').addEventListener('click', function() {
    if (!lastResult) {
        document.getElementById('results').innerHTML = 'Error: No network available';
        return;
    }
    downloadBlob(NarrativeNet.exportNetworkJson(lastResult.network), 'application/json', 'network.json');
});

document.getElementById('exportMetrics').addEventListener('click', function() {
    if (!lastResult) {
        document.getElementById('results').innerHTML = 'Error: No metrics available';
        return;
    }
    downloadBlob(NarrativeNet.exportMetricsCsv(lastResult.metrics), 'text/csv', 'network_metrics.csv');
});
